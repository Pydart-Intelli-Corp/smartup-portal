...
// Students & Parents: submit reschedule/cancel requests
...
export async function POST(req: NextRequest) {
  ...
  if (body.action === 'approve' || body.action === 'reject') {
    ...
    if (body.action === 'reject') {
      ...
      try {
        ...
        await sendEmail({ to: request.requester_email, ...tmpl });
  ...
  if (!['student', 'parent'].includes(role)) {
    return err('Only students and parents can submit requests', 403);
  }
  ...
  // Verify the student/parent is associated with this batch
  if (batch_id) {
  ...
  if (batch_id) {
    ...
    if (batch.length > 0 && batch[0].academic_operator_email) {
      try {
        ...
        await sendEmail({ to: batch[0].academic_operator_email, ...tmpl });
