  ...
  try {
    ...
    const addSession = (
      ...
      if (!recipientMap.has(key)) {
        recipientMap.set(key, { email, name, role, sessions: [], childName });
      }
    ...
    for (const row of studentsRes.rows) {
      const s = row as { batch_id: string; student_email: string; student_name: string; parent_email: string | null; parent_name: string | null };
    ...
    for (const s of sessions) {
      ...
      for (const st of batchStudents) {
        addSession(st.student_email, st.student_name || st.student_email, 'student', sessionInfo);

        if (st.parent_email) {
          addSession(st.parent_email, st.parent_name || st.parent_email, 'parent', sessionInfo, st.student_name || st.student_email);
        }
      }
    }
