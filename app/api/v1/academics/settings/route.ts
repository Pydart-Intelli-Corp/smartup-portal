...
export async function GET() {
  try {
    ...
    if (!settings.subjects) settings.subjects = ['Physics', 'Chemistry', 'Mathematics', 'Social Science', 'English', 'Malayalam', 'Arabic'];
    if (!settings.grades) settings.grades = Array.from({ length: 12 }, (_, i) => String(i + 1));
    if (!settings.sections) settings.sections = ['A', 'B', 'C', 'D', 'E', 'F'];
    if (!settings.boards) settings.boards = ['CBSE', 'ICSE', 'State Board'];
