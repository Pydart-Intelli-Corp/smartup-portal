  ...
  return (
    <DashboardShell role={userRole} userName={userName} userEmail={userEmail} navItems={[
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-400">Manage admission workflow: enquiry → registered → fee confirmed → allocated → active</p>
      </div>
      ...
      <div className="mb-4 relative">
        ...
        <input
          ...
          className="w-full rounded-lg border border-white/10 bg-white/5 py-2 pl-10 pr-4 text-sm text-white placeholder-gray-500 focus:border-blue-500 focus:outline-none"
        />
      </div>

      {/* Refresh */}
      <div className="mb-4 flex justify-end">
        <button onClick={fetchAdmissions} className="flex items-center gap-1 text-xs text-gray-400 hover:text-white">
      ...
      {loading ? (
        ...
        <div className="py-12 text-center text-gray-500">No admissions found</div>
      ) : (
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${STATUS_COLORS[a.status] || 'bg-gray-700 text-gray-300'}`}>
                        {a.status.replace('_', ' ')}
                      </span>
                    </div>
                    ...
                    {a.notes && (
                      <p className="mt-1 text-xs text-gray-500 italic">{a.notes}</p>
                    )}
                  </div>
