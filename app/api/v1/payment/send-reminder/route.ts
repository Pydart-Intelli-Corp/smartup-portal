...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await db.query(
      `SELECT i.*, u.full_name AS student_name, u.parent_email
       FROM invoices i
    ...
    const reminderData = {
      ...
      daysOverdue,
      payLink: `${process.env.NEXTAUTH_URL || 'https://portal.smartupclasses.com'}/student#fees`,
    };
    ...
    if (parentEmail) {
      await sendPaymentReminder({
        ...
        payLink: `${process.env.NEXTAUTH_URL || 'https://portal.smartupclasses.com'}/parent#fees`,
      });
