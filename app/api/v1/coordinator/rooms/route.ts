...
export async function POST(req: NextRequest) {
  ...
  if (!dur || dur < 10 || dur > 300) {
    return NextResponse.json(
      { success: false, error: 'Duration must be between 10 and 300 minutes' },
      { status: 400 }
    );
  }
  if (!maxPart || maxPart < 1 || maxPart > 500) {
    return NextResponse.json(
      { success: false, error: 'Max participants must be between 1 and 500' },
      { status: 400 }
    );
  }
