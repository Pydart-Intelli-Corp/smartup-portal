...
export default function FeesClient({ userName, userEmail, userRole }: Props) {
  const [tab, setTab] = useState<'invoices' | 'structures' | 'create'>('invoices');
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-400 mt-1">Manage fee structures and track payments</p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="rounded-xl border border-gray-700 bg-gray-800/50 p-4">
            <p className="text-xs text-gray-400 mb-1"><IndianRupee className="h-3 w-3 inline mr-1" />Collected</p>
            <p className="text-xl font-bold text-green-400">{money(totalRevenue)}</p>
          </div>
          <div className="rounded-xl border border-gray-700 bg-gray-800/50 p-4">
            <p className="text-xs text-gray-400 mb-1"><Clock className="h-3 w-3 inline mr-1" />Pending</p>
            <p className="text-xl font-bold text-yellow-400">{money(pendingAmount)}</p>
          </div>
          <div className="rounded-xl border border-gray-700 bg-gray-800/50 p-4">
            <p className="text-xs text-gray-400 mb-1"><AlertCircle className="h-3 w-3 inline mr-1" />Overdue</p>
          ...
          {[
            ...
            <button key={t.key} onClick={() => setTab(t.key)}
              className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium transition ${
                tab === t.key ? 'bg-amber-600 text-white' : 'text-gray-400 hover:text-white hover:bg-gray-800'
              }`}>
        ...
        {tab === 'create' && (
          <div className="rounded-xl border border-gray-700 bg-gray-800/50 p-5 space-y-4">
            <h3 className="text-sm font-semibold text-white">Create Fee Structure</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                <label className="text-xs text-gray-400 mb-1 block">Grade (optional)</label>
        ...
        {tab === 'structures' && (
          structures.length === 0 ? (
            <div className="text-center text-gray-500 py-16 text-sm">No fee structures yet</div>
          ) : (
              ...
              {structures.map(s => (
                <div key={s.id} className="rounded-xl border border-gray-700 bg-gray-800/50 p-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full ${s.active ? 'bg-green-500/20 text-green-400' : 'bg-gray-700 text-gray-400'}`}>
            ...
            <div className="overflow-x-auto rounded-xl border border-gray-700">
              <table className="w-full text-sm text-left">
                ...
                <tbody className="divide-y divide-gray-700">
                  {invoices.map(inv => (
                    <tr key={inv.id} className="bg-gray-800/30 hover:bg-gray-800">
                      <td className="px-4 py-3 text-white text-xs">{inv.student_email}</td>
                      ...
                      <td className="px-4 py-3 text-gray-400 text-xs">{new Date(inv.due_date).toLocaleDateString('en-IN')}</td>
