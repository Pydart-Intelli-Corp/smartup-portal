...
import type { SmartUpUser, ApiResponse } from '@/types';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const user: SmartUpUser | null = result.user ?? null;
    ...
    const response = NextResponse.json<ApiResponse<{ user: SmartUpUser }>>(
      {
        success: true,
